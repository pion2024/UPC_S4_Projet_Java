...
public class GamePanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        // 4. Draw Entities (Bottom Layer)
        for (Entity e : world.getEntities()) {
            if (e instanceof Agent || e instanceof Block) continue;
            drawEntity(g2, e);
        }
        
        // 5. Draw Path Preview (Yellow Dotted Line)
        List<Vector2> path = world.getPreviewPath();
        if (path != null && !path.isEmpty()) {
            g2.setColor(new Color(255, 255, 0, 180)); // Yellow
        ...
        // 7. Draw Robot Speech Bubble (Overlay)
    ...
    private void drawSpeechBubble(Graphics2D g2, Robot r, String text) {
        ...
        int y = r.getPosition().y * TILE_SIZE; // Top of robot
    ...
    private void drawEntity(Graphics2D g2, Entity e) {
        ...
        if (e instanceof Trigger) {
            ...
            if (e instanceof Robot) g2.setColor(new Color(220, 20, 60)); 
            else g2.setColor(new Color(30, 144, 255)); 
    ...
    private Entity findEntity(int id) {
        for(Entity e : world.getEntities()) if(e.getId() == id) return e;
        return null;
    }
}